...
public class ManagerProjectService {
    ...
    public ManagerProjectService(ProjectRepository projectRepository, DepartmentRepository departmentRepository) {
    ...
    public ResponseEntity<StandardResponseDTO> saveProject(Project project, BindingResult result) {
        ...
        try {
            ...
            return ResponseEntity.ok(StandardResponseDTO.success("Projeto criado com sucesso!"));
        } catch (DataIntegrityViolationException e) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
