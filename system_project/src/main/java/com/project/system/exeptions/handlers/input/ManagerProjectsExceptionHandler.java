...
import com.project.system.controller.input.ManagerProjectController;

import java.util.HashMap;
import java.util.Map;

@ControllerAdvice(assignableTypes = ManagerProjectController.class)
public class ManagerProjectsExceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/manager/removeProject")) {
            response.put("mensagem", "Você não tem permissão para excluir um projeto.");
        } 
        else if (uri.contains("/input/manager/projects/edit")) {
            response.put("mensagem", "Você não tem permissão para editar um projeto.");
        }
        else if (uri.contains("/input/manager/projects/save")) {
        ...
        if (isAjax || method.equals("POST") || method.equals("DELETE") || 
            (method.equals("GET") && uri.contains("/input/manager/removeProject"))) {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/manager/projects/save")) {
            response.put("mensagem", "Erro durante o cadastro: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/manager/removeProject")) {
            response.put("mensagem", "Erro durante a remoção: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/manager/projects/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/manager/removeProject")) {
