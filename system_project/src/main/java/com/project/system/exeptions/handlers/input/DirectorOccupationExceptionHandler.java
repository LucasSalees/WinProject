...
import com.project.system.controller.input.DirectorOccupationController;

import java.util.HashMap;
import java.util.Map;

@ControllerAdvice(assignableTypes = DirectorOccupationController.class)
public class DirectorOccupationExceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/director/removeOccupation")) {
        	response.put("mensagem", "Você não tem permissão para excluir uma profissão.");
        } 
        else if (uri.contains("/input/director/occupations/edit")) {
            response.put("mensagem", "Você não tem permissão para editar uma profissão.");
        }
        else if  (uri.contains("/input/director/occupations/save")) {
            response.put("mensagem", "Você não tem permissão para editar uma profissão.");
        }
        else {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/director/occupations/save")) {
            response.put("mensagem", "Erro durante o cadastro: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/director/removeOccupation")) {
            response.put("mensagem", "Erro durante a remoção: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/director/occupations/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/director/removeOccupation")) {
