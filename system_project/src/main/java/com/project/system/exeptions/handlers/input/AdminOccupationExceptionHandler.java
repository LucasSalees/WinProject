...
import com.project.system.controller.input.AdminOccupationController;

import java.util.HashMap;
import java.util.Map;

@ControllerAdvice(assignableTypes = AdminOccupationController.class)
public class AdminOccupationExceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/admin/removeOccupation")) {
        	response.put("mensagem", "Você não tem permissão para excluir uma profissão.");
        } 
        else if (uri.contains("/input/admin/occupations/edit")) {
            response.put("mensagem", "Você não tem permissão para editar uma profissão.");
        }
        else if  (uri.contains("/input/admin/occupations/save")) {
            response.put("mensagem", "Você não tem permissão para editar uma profissão.");
        }
        else {
        ...
        if (isAjax || method.equals("POST") || method.equals("DELETE") || 
            (method.equals("GET") && uri.contains("/input/admin/removeOccupation"))) {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/admin/occupations/save")) {
            response.put("mensagem", "Erro durante o cadastro: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/admin/removeOccupation")) {
            response.put("mensagem", "Erro durante a remoção: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/admin/occupations/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/admin/removeOccupation")) {
