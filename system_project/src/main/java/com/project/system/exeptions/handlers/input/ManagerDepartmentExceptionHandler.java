...
import com.project.system.controller.input.ManagerDepartmentController;

import java.util.HashMap;
import java.util.Map;

@ControllerAdvice(assignableTypes = ManagerDepartmentController.class)
public class ManagerDepartmentExceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/manager/removeDepartment")) {
            response.put("mensagem", "Você não tem permissão para excluir um departamento.");
        } 
        else if (uri.contains("/input/manager/departments/edit")) {
            response.put("mensagem", "Você não tem permissão para editar um departamento.");
        }
        else if (uri.contains("/input/manager/departments/save")) {
        ...
        if (isAjax || method.equals("POST") || method.equals("DELETE") || 
            (method.equals("GET") && uri.contains("/input/manager/removeDepartment"))) {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/manager/departments/save")) {
            response.put("mensagem", "Erro durante o cadastro: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/manager/removeDepartment")) {
            response.put("mensagem", "Erro durante a remoção: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/manager/departments/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/manager/removeDepartment")) {
