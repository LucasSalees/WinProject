...
import com.project.system.controller.input.AdminFunctionController;

import java.util.HashMap;
import java.util.Map;

@ControllerAdvice(assignableTypes = AdminFunctionController.class)
public class AdminFunctionExceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/admin/removeFunction")) {
            response.put("mensagem", "Você não tem permissão para excluir uma função.");
        } 
        else if (uri.contains("/input/admin/functions/edit")) {
            response.put("mensagem", "Você não tem permissão para editar uma função.");
        }
        else if  (uri.contains("/input/admin/functions/save")) {
            response.put("mensagem", "Você não tem permissão para editar uma função.");
        }
        else {
        ...
        if (isAjax || method.equals("POST") || method.equals("DELETE") || 
            (method.equals("GET") && uri.contains("/input/admin/removeFunction"))) {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/admin/functions/save")) {
            response.put("mensagem", "Erro ao cadastrar função: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/admin/removeFunction")) {
            response.put("mensagem", "Erro ao remover função: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/admin/functions/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/admin/removeFunction")) {
