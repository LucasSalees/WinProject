...
import com.project.system.controller.input.DirectorProjectController;

import java.util.HashMap;
import java.util.Map;

@ControllerAdvice(assignableTypes = DirectorProjectController.class)
public class DirectorProjectsExceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/director/removeProject")) {
            response.put("mensagem", "Você não tem permissão para excluir um projeto.");
        } 
        else if (uri.contains("/input/director/projects/edit")) {
            response.put("mensagem", "Você não tem permissão para editar um projeto.");
        }
        else if (uri.contains("/input/director/projects/save")) {
        ...
        if (isAjax || method.equals("POST") || method.equals("DELETE") || 
            (method.equals("GET") && uri.contains("/input/director/removeProject"))) {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/director/projects/save")) {
            response.put("mensagem", "Erro durante o cadastro: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/director/removeProject")) {
            response.put("mensagem", "Erro durante a remoção: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/director/projects/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/director/removeProject")) {
