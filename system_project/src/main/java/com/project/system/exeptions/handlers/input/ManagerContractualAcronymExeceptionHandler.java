...
public class ManagerContractualAcronymExeceptionHandler {
    ...
    public Object handleAccessDeniedException(HttpServletRequest request, AccessDeniedException ex) {
        ...
        if (uri.contains("/input/manager/removeContractualAcronym")) {
            response.put("mensagem", "Você não tem permissão para excluir uma sigla contratual.");
        } 
        else if (uri.contains("/input/manager/contractualAcronym/edit")) {
            response.put("mensagem", "Você não tem permissão para editar uma sigla contratual.");
        }
        else if (uri.contains("/input/manager/contractualAcronym/saveContractualAcronym")) {
        ...
        if (isAjax || method.equals("POST") || method.equals("DELETE") || 
            (method.equals("GET") && uri.contains("/input/manager/removeContractualAcronym"))) {
    ...
    public ResponseEntity<Map<String, String>> handleAllExceptions(HttpServletRequest request, Exception ex) {
        ...
        if (request.getRequestURI().contains("/input/manager/contractualAcronym/save")) {
            response.put("mensagem", "Erro durante o cadastro: " + ex.getMessage());
        } 
        else if (request.getRequestURI().contains("/input/manager/removeContractualAcronym")) {
            response.put("mensagem", "Erro durante a remoção: " + ex.getMessage());
        }
        else if (request.getRequestURI().contains("/input/manager/contractualAcronym/edit")) {
    ...
    public ResponseEntity<Map<String, String>> handleDataIntegrityViolationException(
        ...
        if (request.getRequestURI().contains("/input/manager/removeContractualAcronym")) {
