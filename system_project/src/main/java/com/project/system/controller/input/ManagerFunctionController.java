...
public class ManagerFunctionController {
    ...
    @GetMapping("/input/manager/functions/register")
    ...
    public ModelAndView register(Function function, Authentication authentication) {
        ...
        ModelAndView mv = new ModelAndView("input/manager/functions/register");
    ...
    @GetMapping("/input/manager/functions/print/{functionId}")
	@PreAuthorize("hasAuthority('FUNCTION_LIST')")
	public ModelAndView printDepartment(@PathVariable Long functionId, Authentication authentication) {
		...
		ModelAndView mv = new ModelAndView("input/manager/functions/printOne");
		mv.addObject("LoggedUser", loggedUser);
		mv.addObject("function", function);
		mv.addObject("dataAtual", new java.util.Date());
		return mv;
    ...
    public ModelAndView editFunction(@PathVariable("functionId") Long functionId, Authentication authentication) {
        ...
        if (functionOpt.isEmpty()) {
            return new ModelAndView("redirect:/input/manager/functions/list");
        }

        Function function = functionOpt.get();

        ModelAndView mv = new ModelAndView("input/manager/functions/edit");
        mv.addObject("LoggedUser", loggedUser);
        mv.addObject("function", function);
        return mv;
    }

    @GetMapping("/input/manager/removeFunction/{functionId}")
    ...
    @PostMapping("/input/manager/functions/edit")
    ...
    @PostMapping("/input/manager/functions/save")
    @PreAuthorize("hasAuthority('FUNCTION_REGISTER')")
    @ResponseBody
