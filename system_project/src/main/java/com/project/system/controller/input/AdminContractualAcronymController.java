...
public class AdminContractualAcronymController {
	...
	@PreAuthorize("hasAuthority('CONTRACTUAL_ACRONYM_LIST')")
	public ModelAndView printContractualAcronym(@PathVariable Long contractualAcronymId, Authentication authentication) {

		User loggedUser = AuthenticationUtils.getLoggedUser(authentication);
		ContractualAcronym acronym = contractualAcronymService.getContractualAcronymById(contractualAcronymId)
	...
	public ModelAndView editContractualAcronym(@PathVariable("acronymId") Long acronymId, Authentication authentication) {
		...
		if (acronymOpt.isEmpty()) {
			return new ModelAndView("redirect:/input/admin/acronymOpt/list");
		}

		ContractualAcronym acronym = acronymOpt.get();

		ModelAndView mv = new ModelAndView("input/admin/acronymOpt/edit");
		mv.addObject("LoggedUser", loggedUser);
		mv.addObject("acronym", acronym);
		return mv;
	}
