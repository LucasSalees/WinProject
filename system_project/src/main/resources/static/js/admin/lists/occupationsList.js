...
async function loadOccupations(resetTable = false, sortBy = 'occupationCBO', sortDirection = 'asc') {
    ...
    try {
        ...
        const response = await fetch(`/input/admin/occupations/page?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(currentFilter)}&sortBy=${currentSortBy}&sortDirection=${currentSortDirection}`);
        ...
        data.content.forEach(occupation => {
            ...
            row.innerHTML = `
			    <td class="text-left">
			        <a href="/input/admin/occupations/edit/${occupation.occupationId}" class="row-link">${occupation.occupationId}</a>
			    </td>
			    <td class="text-left">
			        <a href="/input/admin/occupations/edit/${occupation.occupationId}" class="row-link">${occupation.occupationName}</a>
			    </td>
			    <td class="text-left">
			        <a href="/input/admin/occupations/edit/${occupation.occupationId}" class="row-link">${occupation.occupationCBO}</a>
			    </td>
			    <td class="text-left">
			        <a href="/input/admin/occupations/edit/${occupation.occupationId}" class="row-link">${occupation.occupationType}</a> <!-- label em português -->
			    </td>
			`;;
