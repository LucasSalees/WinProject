...
async function loadAcronyms(resetTable = false) {
    ...
    try {
        ...
        const response = await fetch(`/input/admin/acronyms/page?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(filter)}`);
        ...
        data.content.forEach(acronym => {
            ...
            row.innerHTML = `
                <td class="text-left">
                    <a href="/input/admin/acronyms/edit/${acronym.acronymId}" class="row-link">${acronym.acronymId}</a>
                </td>
                <td class="text-left">
                    <a href="/input/admin/acronyms/edit/${acronym.acronymId}" class="row-link">${acronym.contractualAcronymName}</a>
                </td>
                <td class="text-left">
                    <a href="/input/admin/acronyms/edit/${acronym.acronymId}" class="row-link">${acronym.acronym}</a>
                </td>
            `;
