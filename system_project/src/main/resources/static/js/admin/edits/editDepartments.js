...
form.addEventListener('submit', async function(event) {
    ...
    fetch('/input/admin/departments/edit', {
        method: 'POST',
        body: formData
    })
    .then(response => {
	...
	.then(data => {
	    if (data.status === "success") {
	        showMessageModal(data.mensagem, true, {
	            redirectUrl: "/input/admin/departments/list"
	        });
	    }
	})
	.catch(error => {
...
function imprimirFormulario() {
    try {
		...
		const dados = {
		    ...
		    departmentManager: getValue('departmentManager'),
			departmentEmail: getValue('departmentEmail'),
			departmentTel: getValue('departmentTel')
		};
        ...
        <html>
        <head>
            <title>WinChurch - Relatório de Departamento</title>
            <style>
                body {
