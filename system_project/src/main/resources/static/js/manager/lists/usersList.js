...
async function loadUsers(resetTable = false) {
    ...
    try {
        ...
        const response = await fetch(`/input/manager/users/page?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(filter)}`);
        ...
        data.content.forEach(user => {
            ...
            row.innerHTML = `
                <td class="text-left">
                    <a href="/input/manager/users/edit/${user.userId}" class="row-link">${user.userId}</a>
                </td>
                <td class="text-left">
                    <a href="/input/manager/users/edit/${user.userId}" class="row-link">${user.userName}</a>
                </td>
                <td class="text-left">
                    <a href="/input/manager/users/edit/${user.userId}" class="row-link">${user.userEmail}</a>
                </td>
				<td class="text-left">
				    <a href="/input/manager/users/edit/${user.userId}" class="row-link">${user.userRole}</a>
				</td>
                <td class="text-left">
                    <a href="/input/manager/users/edit/${user.userId}" class="row-link">${userFunction}</a>
                </td>
            `;
