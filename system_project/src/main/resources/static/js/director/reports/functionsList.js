...
async function loadFunctions(resetTable = false) {
    ...
    try {
        ...
        const response = await fetch(`/input/director/reports/pageFunction?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(filter)}`);
        ...
        data.content.forEach(func => {
            ...
            row.innerHTML = `
                <td class="text-left">
                    <a href="/input/director/reports/editFunction/${func.functionId}" class="row-link">${func.functionId}</a>
                </td>
                <td class="text-left">
                    <a href="/input/director/reports/editFunction/${func.functionId}" class="row-link">${func.functionName}</a>
                </td>
            `;
