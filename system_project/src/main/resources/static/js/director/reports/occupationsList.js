...
async function loadOccupations(resetTable = false, sortBy = 'occupationCBO', sortDirection = 'asc') {
    ...
    try {
		...
		const response = await fetch(
		    `/input/director/reports/pageOccupation?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(filter)}&sortBy=${sortBy}&sortDirection=${sortDirection}`
		);
        ...
        data.content.forEach(occupation => {
            ...
            row.innerHTML = `
				...
				<td class="text-left">
				    <a href="/input/director/reports/editOccupation/${occupation.occupationId}" class="row-link">${occupation.occupationType}</a>
				</td>				
            `;
