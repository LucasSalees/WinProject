...
async function loadFunctions(resetTable = false) {
    ...
    try {
        ...
        const response = await fetch(`/input/director/functions/page?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(filter)}`);
        ...
        data.content.forEach(func => { // 'func' para evitar conflito com a palavra reservada 'function'
            ...
            row.innerHTML = `
                <td class="text-left">
                    <a href="/input/director/functions/edit/${func.functionId}" class="row-link">${func.functionId}</a>
                </td>
                <td class="text-left">
                    <a href="/input/director/functions/edit/${func.functionId}" class="row-link">${func.functionName}</a>
                </td>
            `;
