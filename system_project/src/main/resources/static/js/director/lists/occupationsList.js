...
async function loadOccupations(resetTable = false, sortBy = 'occupationCBO', sortDirection = 'asc') {
    ...
    try {
        ...
        const response = await fetch(`/input/director/occupations/page?page=${currentPage}&size=${pageSize}&filter=${encodeURIComponent(currentFilter)}&sortBy=${currentSortBy}&sortDirection=${currentSortDirection}`);
		...
		data.content.forEach(occupation => {
            ...
            row.innerHTML = `
			    ...
			    <td class="text-left">
			        <a href="/input/director/occupations/edit/${occupation.occupationId}" class="row-link">${occupation.occupationType}</a> <!-- label em português -->
			    </td>
			`;;
