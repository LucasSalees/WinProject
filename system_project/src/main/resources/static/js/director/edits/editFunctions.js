...
form.addEventListener('submit', async function(event) {
    ...
    fetch('/input/director/functions/edit', {
        method: 'POST',
        body: formData
    })
    .then(response => {
	...
	.then(data => {
	    if (data.status === "success") {
	        showMessageModal(data.mensagem, true, {
	            redirectUrl: "/input/director/functions/list"
	        });
	    }
	})
	.catch(error => {
...
function imprimirFormulario() {
    try {
		...
		const dados = {
		    functionId: getValue('functionId'),
		    functionName: getValue('functionName'),
		    functionManager: getValue('functionManager'),
			functionEmail: getValue('functionEmail'),
			functionTel: getValue('functionTel')
		};
        ...
        <body>
            <div class="header">
                ...
                <h2 class="header-title">Relatório de Função</h2>
            </div>
			...
			<div class="user-data">
               <h4>Informações da Função</h4>
               <p><strong>Código:</strong> ${dados.functiontId}</p>
               <p><strong>Nome:</strong> ${dados.functionName}</p>
               <p><strong>Gerente:</strong> ${dados.functionManager}</p>
			   <p><strong>Email:</strong> ${dados.functionEmail}</p>
			   <p><strong>Telefone:</strong> ${dados.functionTel}</p>
           </div>
